...
export class InvoiceGenerator {
    private static validateInvoiceData(data: InvoiceData): { isValid: boolean; errors: string[] } {
        const errors: string[] = []

        if (!data.orderId?.trim()) errors.push('Order ID is required')
        ...
        if (data.total <= 0) errors.push('Total amount must be greater than 0')

        return { isValid: errors.length === 0, errors }
    }

    private static sanitizeText(text: string): string {
        return text?.toString().trim().slice(0, 100) || ''
    }
    ...
    static generateInvoicePDF(invoiceData: InvoiceData): void {
        try {
            ...
            if (!validation.isValid) {
                log.error('Invalid invoice data', { errors: validation.errors }, 'InvoiceGenerator')
                throw new Error(`Invalid invoice data: ${validation.errors.join(', ')}`)
            }

            log.info('Generating invoice PDF', { orderId: invoiceData.orderId }, 'InvoiceGenerator')
            ...
            doc.text(formatCurrency(invoiceData.subtotal), 185, yPosition, { align: 'right' })

            if (invoiceData.tax && invoiceData.tax > 0) {
                yPosition += 6
                doc.text('Tax:', totalsX, yPosition)
                doc.text(formatCurrency(invoiceData.tax), 185, yPosition, { align: 'right' })
            }

            if (invoiceData.shipping && invoiceData.shipping > 0) {
                yPosition += 6
                doc.text('Shipping:', totalsX, yPosition)
                doc.text(formatCurrency(invoiceData.shipping), 185, yPosition, { align: 'right' })
            }
            ...
            doc.text(formatCurrency(invoiceData.total), 185, yPosition + 2, { align: 'right' })
            ...
            // Save the PDF
            const fileName = `Invoice_${this.sanitizeText(invoiceData.orderId)}_${new Date().toISOString().split('T')[0]}.pdf`
            doc.save(fileName)
            ...
            log.error('Failed to generate invoice PDF', error, 'InvoiceGenerator')
            throw error
        }
    }
}
